            ...
            for f in range(F):  
                for h in range(H_out): 
                    for w_idx in range(W_out): 
                        ...
                        out[n, f, h, w_idx] = torch.sum(receptive_field * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
          ...
          for c in range(C):
            for h in range(H_out):
              for w in range(W_out):
                ...
                out[n, c, h, w] = torch.max(pool_region)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C): 
            for h in range(H_out):  
              for w in range(W_out):  
                ...
                _, max_index = torch.max(pool_region.reshape(-1), dim=0)
                max_row = max_index // pool_width
                max_col = max_index % pool_width
                
                dx[n, c, h_start+max_row, w_start+max_col] = dout[n, c, h, w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        linear_input_size = num_filters * (H // 2) * (W // 2)

        self.params['W2'] = torch.randn(linear_input_size, hidden_dim, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        linear_input, cache_conv = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        hidden, cache_hidden = Linear_ReLU.forward(linear_input, W2, b2)
        scores, cache_linear = Linear.forward(hidden, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dh, dW3, db3 = Linear.backward(dscores, cache_linear)
        dW3 += 2 * self.reg * self.params['W3']
        dlinear_input, dW2, db2 = Linear_ReLU.backward(dh, cache_hidden)
        dW2 += 2 * self.reg * self.params['W2']
        dX, dW1, db1 = Conv_ReLU_Pool.backward(dlinear_input, cache_conv)
        grads['W1'] = dW1
        grads['b1'] = db1
        grads['W2'] = dW2
        grads['b2'] = db2
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, num_filter in enumerate(num_filters):
          if weight_scale == 'kaiming':
            self.params[f'W{i+1}'] = kaiming_initializer(C, num_filter, K=3, relu=True, device=device, dtype=dtype)
          else:
            self.params[f'W{i+1}'] = torch.randn(num_filter, C, 3, 3, dtype=dtype, device=device) * weight_scale
        ...
        fc_input_dim = C * H * W
        if weight_scale == 'kaiming':
          self.params[f'W{self.num_layers}'] = kaiming_initializer(fc_input_dim, num_classes, relu=False, device=device, dtype=dtype)
        else:
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          ...
          if self.batchnorm == True:
            ...
            if i in self.max_pools:
              current_input, cache = Conv_BatchNorm_ReLU_Pool.forward(current_input, W, b, gamma, beta, conv_param, bn_param, pool_param)
            else:
              current_input, cache = Conv_BatchNorm_ReLU.forward(current_input, W, b, gamma, beta, conv_param, bn_param)
          else:
            if i in self.max_pools:
              current_input, cache = Conv_ReLU_Pool.forward(current_input, W, b, conv_param, pool_param)
            else:
              current_input, cache = Conv_ReLU.forward(current_input, W, b, conv_param)
          caches[f'layer{i+1}'] = cache
        W_fc, b_fc = self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}']
        scores, cache = Linear.forward(current_input, W_fc, b_fc)
        ...
        ###################################################################
        # Replace "pass" statement with your code
        data_loss, dh = softmax_loss(scores, y)
        reg_loss = 0.0
        ...
        dh, dW, db = Linear.backward(dh, caches[f'layer{self.num_layers}'])
        ...
        for i in range(self.num_layers - 2, -1, -1):
          ...
          if self.batchnorm == True:
            if i in self.max_pools:
              dh, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dh, cache)
            else:
              dh, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dh, cache) 
            grads[f'gamma{i+1}'] = dgamma
            grads[f'beta{i+1}'] = dbeta
          else:
            if i in self.max_pools:
              dh, dW, db = Conv_ReLU_Pool.backward(dh, cache)
            else:
              dh, dW, db = Conv_ReLU.backward(dh, cache)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(
        input_dims=(3, 32, 32),
        num_filters=[32, 64, 128, 256, 512],  # 减少层数，增加filter深度
        max_pools=[2, 4],  # 更频繁池化
        num_classes=10,
        reg=5e-5, 
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        fan_in = Din
        std = torch.sqrt(torch.tensor(gain / fan_in, dtype=dtype))
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dx_mu1 = dx_normalized * ivar
        dx_mu2 = dx_normalized.sum(dim=0) * (-ivar)
        
        dx_var = dx_normalized * (x - mu) * (-0.5 * ivar**3)
        dx_var = dx_var.sum(dim=0)
        
        dx_x1 = dx_mu1
        dx_x2 = dx_mu2 / N
        dx_x3 = 2 * dx_var * (x - mu) / N
        
        dx = dx_x1 + dx_x2 + dx_x3
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dx = gamma * (dout - dout.mean(dim=0) - x_normalized * (dout * x_normalized).mean(dim=0)) / std
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
