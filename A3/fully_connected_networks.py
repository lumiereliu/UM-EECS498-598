...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        correct_log_probs = -torch.log(probs[range(X.shape[0]), y])
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, len(layer_dims)):
            self.params[f'W{i}'] = torch.randn(layer_dims[i-1], layer_dims[i], dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            W = self.params[f'W{i}']
            b = self.params[f'b{i}']
            current_input, cache = Linear_ReLU.forward(current_input, W, b)
            caches[f'layer{i}'] = cache
            if self.use_dropout:
              current_input, dropout_cache = Dropout.forward(current_input, self.dropout_param)
              caches[f'dropout_layer{i}'] = dropout_cache
        W_last = self.params[f'W{self.num_layers}']
        b_last = self.params[f'b{self.num_layers}']
        scores, cache = Linear.forward(current_input, W_last, b_last)
        ...
        dh = probs.clone()
        dh[range(X.shape[0]), y] -= 1
        dh /= X.shape[0]
        for i in range(self.num_layers, 0, -1):
          if i == self.num_layers:
            dh, dW, db = Linear.backward(dh, caches[f'layer{i}'])
          else:
            if self.use_dropout:
              dh = Dropout.backward(dh, caches[f'dropout_layer{i}'])
              dh, dW, db = Linear_ReLU.backward(dh, caches[f'layer{i}'])
            else:
              dh, dW, db = Linear_ReLU.backward(dh, caches[f'layer{i}'])
