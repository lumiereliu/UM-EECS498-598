...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = ((scores_real - 1) ** 2 / 2).mean()
    loss_fake = (scores_fake ** 2 / 2).mean()
    loss = loss_real + loss_fake
...
def build_dc_classifier():
    ...
    model = nn.Sequential(
        ...
        nn.LeakyReLU(negative_slope=0.01), nn.MaxPool2d(kernel_size=2, stride=2),
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        nn.Linear(in_features=noise_dim, out_features=1024),
        nn.ReLU(), nn.BatchNorm1d(num_features=1024),
        nn.Linear(in_features=1024, out_features=7 * 7 * 128),
        nn.ReLU(), nn.BatchNorm1d(num_features=7 * 7 * 128),
