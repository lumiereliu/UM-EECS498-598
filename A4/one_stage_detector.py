...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, features in feats_per_fpn_level.items():
            ...
            cls_feats = self.stem_cls(features)
            cls_logits = self.pred_cls(cls_feats)
            
            box_feats = self.stem_box(features)
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.stack([xc - x1, yc - y1, x2 - xc, y2 - yc], dim=1)
    deltas /= stride
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.sqrt(
        torch.minimum(left, right) * torch.minimum(top, bottom) /
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        level_pred = self.pred_net(fpn_features)
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = level_pred
        ######################################################################
        ...
        shape_per_fpn_level = {
            level_name: fpn_features[level_name].shape 
            for level_name in fpn_features.keys()
        }
        ...
        for idx in range(images.shape[0]):
            img_gt_boxes = gt_boxes[idx]
            img_matched_boxes = fcos_match_locations_to_gt(
                locations_per_fpn_level, self.backbone.fpn_strides, img_gt_boxes
            )
            matched_gt_boxes.append(img_matched_boxes)
        ...
        for idx in range(images.shape[0]):
            img_matched_deltas = {
                level_name: fcos_get_deltas_from_locations(
                    ...
                    matched_gt_boxes[idx][level_name],
                    self.backbone.fpn_strides[level_name]
                )
                for level_name in locations_per_fpn_level.keys()
            }
            matched_gt_deltas.append(img_matched_deltas)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            class_scores, level_pred_classes = level_pred_scores.max(dim=1)

            # Step 2:
            # Replace "pass" statement with your code
            keep_indices = class_scores > test_score_thresh
            level_pred_scores = class_scores[keep_indices]
            level_pred_classes = level_pred_classes[keep_indices]
            level_deltas = level_deltas[keep_indices]
            level_locations = level_locations[keep_indices]
