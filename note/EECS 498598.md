...
1. K-Nearest Neighbor
   ...
   - K: Hyper Parameter
     ...
     - too low: overfitting
     - too high: useless
     - Cross Validation Data set: Split data into folds![](./images/cross-validation.png)
...
1. Interpreting a Linear Classifier
   ...
   - Algebraic Viewpoint:
     - Bias Trick![bias-trick-before](./images/bias-trick-before.png)![bias-trick-after](./images/bias-trick-after.png)
   - Visual Viewpoint:
...
- Backprop: Higher-Order Derivatives![higher-order derivatives](./images/higher-order derivatives.png)
...
- Convolution layers
  ...
  ![convolution-layers](./images/convolution-layers.png)
  ...
  - **Problem**: For large images we need many layers for each output to see the whole image
    ...
    ![convolution-summary](./images/convolution-summary.png)
...
- Layer Normalization / Instance Normalization: change dimension view![normalization](./images/normalization.png)

## 08 CNN Architectures

- AlexNet: memory / params / flop![AlexNet](./images/AlexNet.png)
- VGG:![VGG](./images/VGG.png)
...
- GoogleNet: ![GoogleNet](./images/GoogleNet.png)
...
- ResNet:![ResNet](./images/ResNet.png)
  ...
  - Bottleneck Block![ResNet-Bottleneck](./images/ResNet-Bottleneck.png)
...
- RNN Computational Graph
  - One to One
  - One to Many![RNN-OnetoMany](./images/RNN-OnetoMany.png)
  - Many to Many![RNN-ManytoMany](./images/RNN-ManytoMany.png)
  - Many to One![RNN-ManytoOne](./images/RNN-ManytoOne.png)
  - seq2seq(Many to one) + (One to many)![RNN-seq2seq](./images/RNN-seq2seq.png)
  
- Truncated Backpropagation Through Time

- CNN+RNN![CNN+RNN](./images/CNN+RNN.png)
...
- Vanilla RNN Gradient Flow
  ...
  - Multiply W for many times
    ...
    - Largest singular value < 1:  Vanishing gradients
      ...
      Solution: Change RNN architecture: Long Short Term Memory(LSTM)![LSTMGradientFlow](./images/LSTMGradientFlow.png)
...
- seq2seq with RNNs![seq2seqwithRNNs](./images/seq2seqwithRNNs.png)

- seq2seq with RNNs and Attention![seq2seqwithRNNsandAttention](./images/seq2seqwithRNNsandAttention.png)

- Image Captioning with RNNs and Attention![ImageCaptioningwithAttention](./images/ImageCaptioningwithAttention.png)

- Attention Layer![Attention-Layer](./images/Attention-Layer.png)

- Self-Attention Layer![Self-Attention-Layer](./images/Self-Attention-Layer.png)

  - Self attention doesn't know the order of the vectors it is processing

    - concatenate input with positional encoding

    - Masked Self-Attention Layer![MaskedSelfAttentionLayer](./images/MaskedSelfAttentionLayer.png)
...
- The Transformer![TransformerBlock](./images/TransformerBlock.png)
...
- DeepDream: Amplify Existing Features![DeepDream](./images/DeepDream.png)

- Neural Sytle Transfer: Feature Inversion + Texture Synthesis

  - **Feature Inversion**![FeatureInversion](./images/FeatureInversion.png)

    *Reconstructing from higher layers lost more features*

  - **Texture Synthesis**![TextureSynthesis](./images/TextureSynthesis.png)
...
- R-CNN: Region-Based CNN![R-CNNtesttime](./images/R-CNNtesttime.png)
  ...
  - Mean Average Precision (mAP)![mAP](./images/mAP.png)
...
- Fast R-CNN![FastR-CNN](./images/FastR-CNN.png)

  - Cropping Features: RoI Pool![RoIPool](./images/RoIPool.png)

    **Problem**: Slight misalignment due to snapping, different-sized subregion is weird

  - **Solution**: RoI Align![RoI-Align](./images/RoI-Align.png)

  **Problem**: Runtime dominated by region proposals

- **Solution**: Faster R-CNN: Learnable Region Proposals![FasterR-CNN](./images/FasterR-CNN.png)
...
- Semantic Segmentation
  ...
  - Fully Convolutional Network![FullyConvNet](./images/FullyConvNet.png)
    - Downsampling:
      - Pooling
      - strided
      - convolution
    - Upsampling:
      - Unpooling![Unpooling](./images/Unpooling.png)
      - Bilinear Interpolation (also used in RoI Align)![BilinearInterpolation](./images/BilinearInterpolation.png)
      - Bicubic Interpolation![BicubicInterpolation](./images/BicubicInterpolation.png)
      - Max Unpooling![MaxUnpooling](./images/MaxUnpooling.png)
      - Learnable Upsampling: Transposed Convolution![TransposedConv](./images/TransposedConv.png)
...
- Instance Segmentation: Detect all objects in the image, and identify the pixels that belong to each object (Only things)
  ...
  - Mask R-CNN![MaskR-CNN](./images/MaskR-CNN.png)
...
- 3D Shape Representations
  ...
  - Depth Map
    ...
    - **Solution**: Scale invariant
      ...
      $$
      ![ScaleInvariantLoss](./images/ScaleInvariantLoss.png)
  ...
  - Voxels Grid
    ...
    - Processing Voxel Inputs: 3D Convolution![3D Convolution-1](./images/3D Convolution-1.png)

    - Generating Voxel Shapes:

      - 3D Convolution![3D Convolution-2](./images/3D Convolution-2.png)

      - Voxel Tubes![Voxel Tubes](./images/Voxel Tubes.png)

      - **Problem**: Memory Usage

        Storing $1024^3$ voxel grid takes 4GB of memory!

      - **Solution1**: Oct-Trees![Oct-Trees](./images/Oct-Trees.png)

      - **Solution2**: Nested Shape Layers![Nested Shape Layers](./images/Nested Shape Layers.png)
  ...
  - Pointcloud
    ...
    - Processing Pointcloud Inputs: PointNet![ProcessingPointcloudInput](./images/ProcessingPointcloudInput.png)
    - Generating Pointcloud Outputs:![GeneratingPointcloudOutputs](./images/GeneratingPointcloudOutputs.png)
    - Predicting Point Clouds: Chamfer distance![ChamferDistance](./images/ChamferDistance.png)
  
  - Triangle Mesh
  
    - Represent a 3D shape as  a set of triangles![TriangleMesh](./images/TriangleMesh.png)
    - Pixel2Mesh
      - **Idea #1: Iterative mesh refinement**![IterativeMeshRefinement](./images/IterativeMeshRefinement.png)
        - Graph Convolution![GraphConvolution](./images/GraphConvolution.png)
      - **Idea #2: Vertex-Aligned Features**![Vertex-AlignedFeatures](./images/Vertex-AlignedFeatures.png)
        - Loss Function![PredictingMeshesLossFunction](./images/PredictingMeshesLossFunction.png)
...
- 3D Datasets![3DDatasets](./images/3DDatasets.png)

- Mesh R-CNN![MeshRCNN](./images/MeshRCNN.png)
...
- Video = 2D + Time
  ...
  - Video Classification
    ...
    - Late Fusion
      ...
      - with FC layers![LateFusionWithFC](./images/LateFusionWithFC.png)
      - with pooling![LateFusionwithPooling](./images/LateFusionwithPooling.png)

      - **Problem**: Hard to compare low-level motion between frames

    - **Solution**: Early Fusion![EarlyFusion](./images/EarlyFusion.png)

      - **Problem #1**: One layer of temporal processing may not be enough
      - **Problem #2**: No Temporal Shift-Invariance![No Temporal Shift-Invariance](./images/No Temporal Shift-Invariance.png)

    - **Solution**: 3D CNN![Videoswith3DCNN](./images/Videoswith3DCNN.png)
- Recognizing Actions from Motion
  - Measuring Motion: Optical Flow
  - Separating Motion and Appearance: Two-Stream Network![Two-StreamNetwork](./images/Two-StreamNetwork.png)
  - Recurrent Convolutional Network![RecurrentConvolutionalNetwork](./images/RecurrentConvolutionalNetwork.png)
    - **Problem**: RNNs are slow for long sequences (can't be parallelized)
  - **Solution**: Spatio-Temporal Self-Attention![NonLocalBlock](./images/NonLocalBlock.png)
- Inflating 2D Networks to 3D (I3D)![I3D](./images/I3D.png)

## 19-20 Generative Models
...
- Generative Models Tree Map![GMTreeMap](./images/GMTreeMap.png)

- Autoregressive Model

  - **Goal**: Write down an explicit function for p(x) = f(x, W)
  - PixelRNN![PixelRNN](./images/PixelRNN.png)
  - PixelCNN![PixelCNN](./images/PixelCNN.png)
...
- Autoencoders
  ...
  - Variational Autoencoders![VAE](./images/VAE.png)
    ...
    - **Solution**: Train another network (encoder) that learns $q_\phi(z|x) \approx p_\theta(z|x)$![KLdivergence](./images/KLdivergence.png)
    
    - train process:![VAETrain](./images/VAETrain.png)
    - Generating Data:![VAEGeneratingData](./images/VAEGeneratingData.png)
    - Image Editing:![VAEImageEditing](./images/VAEImageEditing.png)
    
  - Generative Adversarial Networks![GANloss](./images/GANloss.png)
    ...
    - **Solution**: Right now G is trained to minimize log(1-D(G(z))). Instead train G to minimize -log(D(G(z)))
    - Optimality: (推导过程见视频)![GANoptimality](./images/GANoptimality.png)
